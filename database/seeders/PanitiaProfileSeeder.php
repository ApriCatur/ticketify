...
class PanitiaProfileSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($panitiaUsers as $user) {
            if (PanitiaProfile::where('user_id', $user->id)->exists()) continue;

            $application = RoleApplication::where('user_id', $user->id)
            ...
            $this->command->info("PanitiaProfile created for user #{$user->id} ({$user->name})");
        }
    }
}
