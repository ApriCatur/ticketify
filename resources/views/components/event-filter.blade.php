    ...
    <form action="{{ route($route) }}" method="GET" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-3 flex flex-col gap-2">
            ...
            <div class="relative">
                ...
                <select name="category" class="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-2xl text-white appearance-none focus:ring-2 focus:ring-blue-500 transition-all">
                    <option value="" class="bg-[#18181b]">Semua</option>
                    <option value="Musik" class="bg-[#18181b]" {{ request('category') == 'Musik' ? 'selected' : '' }}>Musik</option>
                    <option value="Seminar" class="bg-[#18181b]" {{ request('category') == 'Seminar' ? 'selected' : '' }}>Seminar</option>
                </select>
