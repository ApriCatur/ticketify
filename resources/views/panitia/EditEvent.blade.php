...
<body
class="bg-[#F8FAFC] text-gray-900 flex flex-col min-h-screen"
    ...
    <main class="flex-1 p-10 overflow-y-auto">
        ...
        @if ($errors->any())
            <div class="mb-6 p-4 bg-red-50 border border-red-200 text-red-600 rounded-2xl text-sm">
                <strong class="block mb-1 font-black uppercase tracking-wide text-xs text-red-600">Gagal Menyimpan Perubahan:</strong>
        ...
        <div class="flex gap-8 mb-8 border-b border-gray-200">
            ...
            <button type="button" @click="activeTab = 'detail'" :class="activeTab === 'detail' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'" class="pb-4 font-bold text-sm transition-all flex items-center gap-2">
                <span class="w-6 h-6 rounded-full bg-gray-50 flex items-center justify-center text-[10px]">2</span> Event Detail
            </button>
            <button type="button" @click="activeTab = 'organiser'" :class="activeTab === 'organiser' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'" class="pb-4 font-bold text-sm transition-all flex items-center gap-2">
                <span class="w-6 h-6 rounded-full bg-gray-50 flex items-center justify-center text-[10px]">3</span> Organiser
            </button>
        </div>
        ...
        <form action="{{ route('panitia.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" id="editEventForm" novalidate>
                ...
                <div class="grid md:grid-cols-2 gap-8">
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-3xl p-[110px] text-center hover:border-blue-500 transition-colors bg-gray-50 group relative overflow-hidden" :class="bannerPreview ? 'p-4' : 'p-[110px]'">
                            <template x-if="!bannerPreview">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <button type="button" onclick="document.getElementById('poster').click()" class="mt-4 px-6 py-2 bg-gray-200 rounded-full text-[10px] font-bold hover:bg-gray-300 transition-all border border-gray-200">Pilih File Baru</button>
                                </div>
                            </template>
                            <template x-if="bannerPreview">
                                <div class="relative">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $event->name) }}" placeholder="Masukkan nama event" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none transition-all" required>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="time" name="time_start" value="{{ old('time_start', $event->time_start) }}" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none transition-all" required>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Waktu Selesai</label>
                                <input type="time" name="time_end" value="{{ old('time_end', $event->time_end) }}" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none transition-all" required>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm">
                    ...
                    <div class="space-y-3">
                        <template x-for="(ticket, index) in ticketRows" :key="index">
                            <div class="grid md:grid-cols-12 gap-4 items-end bg-gray-50 p-4 rounded-2xl border border-gray-200">
                                <div class="md:col-span-4 space-y-2">
                                    <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Jenis Tiket</label>
                                    <input type="text" :name="'ticket_types['+index+'][ticket_type]'" x-model="ticket.ticket_type" placeholder="Contoh: Reguler / VIP" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none" required>
                                </div>
                                <div class="md:col-span-4 space-y-2">
                                    <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Harga (IDR)</label>
                                    <input type="number" :name="'ticket_types['+index+'][price]'" x-model="ticket.price" min="0" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none" required>
                                </div>
                                <div class="md:col-span-3 space-y-2">
                                    <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Stok Tiket</label>
                                    <input type="number" :name="'ticket_types['+index+'][stock]'" x-model="ticket.stock" min="0" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:border-blue-500 outline-none" required>
                                </div>
            ...
            <div x-show="activeTab === 'detail'" class="space-y-8" style="display: none;">
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="6" class="w-full bg-white border border-gray-200 rounded-2xl p-4 text-sm text-gray-900 focus:border-blue-500 outline-none" placeholder="Ceritakan detail acaramu..." required>{{ old('description', $event->description) }}</textarea>
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Syarat & Ketentuan</label>
                    <textarea name="terms" rows="4" class="w-full bg-white border border-gray-200 rounded-2xl p-4 text-sm text-gray-900 focus:border-blue-500 outline-none" placeholder="1. Peserta wajib..." required>{{ old('terms', $event->terms) }}</textarea>
                </div>
            ...
            <div x-show="activeTab === 'organiser'" class="space-y-8" style="display: none;">
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <textarea name="organiser_description" rows="8" class="w-full bg-white border border-gray-200 rounded-2xl p-4 text-sm text-gray-900 focus:border-blue-500 outline-none" placeholder="Profil singkat penyelenggara...">{{ old('organiser_description', $event->organiser_description) }}</textarea>
                    </div>
                    <div class="space-y-4">
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-3xl p-10 text-center bg-gray-50 group" :class="orgPhotoPreview ? 'p-4' : 'p-10'">
                            <template x-if="!orgPhotoPreview">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <button type="button" onclick="document.getElementById('organiser_photo').click()" class="px-6 py-2 bg-gray-200 rounded-full text-[10px] font-bold hover:bg-gray-300 transition-all border border-gray-200">Upload Photo Baru</button>
                                </div>
                            </template>
                            <template x-if="orgPhotoPreview">
                                <div class="relative">
